...
add_torrent_headless() {
  ...
  local base64_content=$(base64 < "$file_path" | tr -d '\n')
  ...
  if mv "$source" "$dest"; then
    ...
    if [ "$HEADLESS_MODE" = true ]; then
      ...
      if ! add_torrent_headless "$filename"; then
        ...
        open -a Transmission "$dest"
      fi
    else
      # GUI mode - open normally
      if open -a Transmission "$dest" 2>/dev/null; then
        success "Opened with Transmission (GUI): ${GREEN}${filename}${NC}"
      else
        warning "Could not open with Transmission. Opening manually..."
        open "$dest"
      fi
    fi
  else
